...
export default function LessonEditor() {
    ...
    try {
      const res = await fetch('/api/generate-audio', {
        ...
        body: JSON.stringify({
          ...
          slideNumber: 0, // вопросы не привязаны к слайдам
        }),
      })
      ...
      // Обновляем аудио URL для вопроса
      const updatedQuestions = [...questions]
      updatedQuestions[questionIndex] = { ...question, audioUrl: data.audioUrl }
  ...
  const generateAudio = async (slideIndex: number) => {
    ...
    try {
      // ИСПРАВЛЕНО: API возвращает JSON, а не blob
      const res = await fetch('/api/generate-audio', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
